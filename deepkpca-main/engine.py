...
@Author: CarpeDiem
@Date: 2023/11/8
@Description: 使用 Pytorch 框架，重新架构
@Improvement:  
"""
...
class engine(nn.Module):
    def __init__(self, N_o, device):
        ...
        # Generator network, G
        self.generator = nn.Sequential(nn.Linear(12, 100),          # # dim(z) = dim(z_SNP) + dim(c) = 54 Now 56
                                       nn.ELU(),
                                       nn.Linear(100, 180), 
    ...
    def encode(self, x_SNP):
        x_SNP = x_SNP.reshape(x_SNP.shape[0], 1, x_SNP.shape[1])         # Become the input of one-dimensional convolution
        return self.encode(x_SNP)
    ...
    def diagnose(self, x_MRI, a, apply_logistic_activation=False):
        ...
        if apply_logistic_activation:
            # y_hat = logit_clf.argmax(dim=1)               # Output category 0 1
