...
class Net1(nn.Module):
    ...
    def __init__(self, nChannels, capacity, x_fdim1, x_fdim2, cnn_kwargs):
        ...
        self.main = nn.Sequential(
            nn.Conv1d(nChannels, capacity, **cnn_kwargs[0]),        # 1, 1, 4, 2, 1
            nn.LeakyReLU(negative_slope=0.2),

            nn.Conv1d(capacity, capacity * 2, **cnn_kwargs[0]),     # 1, 2, 4, 2, 1
            nn.LeakyReLU(negative_slope=0.2),

            nn.Conv1d(capacity * 2, capacity * 4, **cnn_kwargs[1]), # 2, 4, 3, 1
            nn.LeakyReLU(negative_slope=0.2),

            nn.Flatten(),
            nn.Linear(1992, x_fdim1),                               # 64 * 4 * 8 ** 2
            nn.LeakyReLU(negative_slope=0.2),
...
class Net3(nn.Module):
    ...
    def __init__(self, nChannels, capacity, x_fdim1, x_fdim2, cnn_kwargs):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.2),
            nn.Linear(x_fdim1, 1992),
            nn.LeakyReLU(negative_slope=0.2),
            Lin_View(4, 498),  # Unflatten
...
def main(args: DictConfig):
    ...
    X_MRI_train, x_train, C_demo_train, Y_train, S_train, \
    X_MRI_test, x_test, C_demo_test, Y_test, S_test = loaddata.load_dataset(2, ['HC', 'AD'])
    ...
    op1 = phi1(x_train)
    H2, L2 = training_dict["H2_tilde"].to(device), training_dict["L2_tilde"].to(device)     # find multiple principal components
