...
# Load data samples
def load_dataset(fold, task, in_start, in_end, SNP_mapping=True, return_tensor=False):
    mytask = []
    ...
    path = "/data/pat/code/DKPCA-ENGINE/data/744_Data_230418/"      # VBM
    # path = "/data/pat/code/DKPCA-ENGINE/data/749_Data_230418/"      # FS

    # The input data has been encoded and the maximum value of the column is found.
    ...
    # FS
    # X_MRI = np.load(path + '749_FS_230418.npy')[task_idx, :]           # MRI volume features (N, 90)
    # # X_SNP = np.load(path + '744_SNP_DATA_230418.npy')[task_idx, :]      # SNP features (N, 2000)
    # C_sex = np.load(path + '749_Gender_230418.npy')[task_idx]           # Sex codes 'F', 'M' (N, )
    # C_edu = np.load(path + '749_Education_230418.npy')[task_idx]        # Education codes (N, )
    # C_age = np.load(path + '749_Age_230418.npy')[task_idx]              # Age codes (N, )
    # # C_hand = np.load(path + '744_Handedness_230418.npy')[task_idx]      # Hand 'L', 'R' (N, )
    # S_cog = np.load(path + '749_MMSE_230418.npy')[task_idx]             # Cognitive scores (MMSE), (N, )

    data_name = "Gene_VBM.mat"
    # data_name = "Gene_FS.mat"
    data_file = os.path.join(path, data_name)
    ...
    Y_dis = np.eye(np.unique(Y_dis).shape[0])[Y_dis]        # [[1, 0], [0, 1]]
    ...
    C_dmg = np.concatenate((C_sex, C_age[:, None], C_edu[:, None]), -1) # (744, 4)
    ...
    # X_MRI_tr, X_MRI_ts, X_SNP_tr, X_SNP_ts, C_dmg_tr, C_dmg_ts, Y_dis_tr, Y_dis_ts, \
    # S_cog_tr, S_cog_ts = train_test_split(X_MRI, X_SNP, C_dmg, Y_dis, S_cog)
    # MRI normalization
    scaler = MinMaxScaler()
