...
class experiment():
    ...
    def training(self, levels, args_optimizer, ae_weight=None):
        ...
        N_in, N_out = 2000, Y_train.shape[-1]               # Look at the number of columns in Y_train [[0, 1], [1, 0]]    
        ...
        while cost > 1e-10 and t < args_optimizer.maxepochs and terminating_condition(cost, rkm2, optimizer) and epoch < self.num_epochs:  # run epochs until convergence or cut-off
            ...
            with open("CN_AD_loss_23122401.txt", "a+", encoding='utf-8') as f:
        ...
        with torch.no_grad():
            ...
            # file_name = str(self.fold_idx) + '_' + current_time + '_HC_AD_result.txt'
            # file_path = os.path.join(folder_path, file_name)
            with open('AD_MCI_HC_VBM_result_231226.txt', "a+", encoding='utf-8') as f:
                f.write("\n**********************************\n")
...
def main(args: DictConfig):
    task = ['HC', 'MCI', 'AD']          # ['HC', 'AD'], ['AD', 'MCI'], ['HC', 'MCI'], ['AD', 'MCI', 'HC']
    for fold in range(5):               # five-fold cross-validation
        exp = experiment(fold + 1, ['AD', 'HC', 'MCI'])
        nChannels = 1
