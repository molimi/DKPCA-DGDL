    ...
    def training(self, levels, args_optimizer, ae_weight=None):
        ...
        while cost > 1e-10 and t < args_optimizer.maxepochs and terminating_condition(cost, rkm2, optimizer) and epoch < self.num_epochs:  # run epochs until convergence or cut-off
            ...
            for batch in range(num_iters):
                ...
                # xb_MRI_fake, ab = model.generate(z_SNP=torch.cat((H1_tilde, H2_tilde), dim=-1))     # 不嵌入人口统计信息
                ab = model.generate(z_SNP=torch.cat((H1_tilde, H2_tilde), dim=-1), c_demo=cb_demo)
        ...
        with torch.no_grad():
            ...
            A_test = model.generate(Z_SNP_test, C_demo_test)
            ...
            file_name = "./Case4/HC&MCI/" + 'fs_HC_MCI_result_231228.txt'
            # file_path = os.path.join(folder_path, file_name)
