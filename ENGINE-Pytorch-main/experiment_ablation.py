    ...
    def training(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in range(num_iters):
                ...
                # cross_ent = F.binary_cross_entropy_with_logits(eb_SNP_hat_logit, eb_SNP)
                # log_prob_eb_SNP_given_zb_SNP = -torch.sum(cross_ent, dim=-1)
                # log_prob_zb_SNP = loaddata.log_normal_pdf_1(sample=zb_SNP, mean=0., logvar=0., device=self.device)
                # log_q_zb_given_eb_SNP = loaddata.log_normal_pdf_2(sample=zb_SNP, mean=mu, logvar=log_sigma_square, device=self.device)
                # Reconstruction loss
                ...
                yb_clf_hat, sb_reg_hat = model.diagnose1(x_MRI=xb_MRI, apply_logistic_activation=True)

                # Classification loss
                # yb_clf_hat = F.log_softmax(yb_clf_hat)
                # L_clf = F.nll_loss(yb_clf_hat, yb_clf)
                # func_clf = torch.nn.CrossEntropyLoss(reduction="mean")
                # 使用自定义的损失函数
                loss_fn = SigmoidFocalCrossEntropyLoss()
                L_clf = loss_fn(yb_clf, yb_clf_hat)
                # print(yb_clf)
                # print(yb_clf_hat)
                # print(yb_clf_hat.shape, yb_clf.shape)
                # L_clf = func_clf(yb_clf_hat, yb_clf)
        ...
        with torch.no_grad():
            ...
            Y_test_hat, S_test_hat = model.diagnose1(X_MRI_test, True)
            ...
            # 写入文件中
            file_name = "./Final_Result/" + 'fs_HC_MCI_result_231228.txt'
