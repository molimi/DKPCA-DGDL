    ...
    def training(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in range(num_iters):
                ...
                # Classification loss, Use custom loss function
        ...
        with torch.no_grad():
            ...
            # print(Y_test)
            # print(Y_test_hat)
            print(f'Test Accuracy Score: {accuracy_score(np.argmax(Y_test, axis=1), np.argmax(Y_test_hat, axis=1)):>.4f}')
            ...
            with open("CN_AD_result_23122501.txt", "a+", encoding='utf-8') as f:
                f.write("\n**********************************\n")
                f.write(f'Start Training, Fold {self.fold_idx}\n')
                f.write(f'Test AUC: {roc_auc_score(Y_test, Y_test_hat):>.4f}\n')
                f.write(f'Test Accuracy Score: {accuracy_score(np.argmax(Y_test, axis=1), np.argmax(Y_test_hat, axis=1)):>.4f}\n')
